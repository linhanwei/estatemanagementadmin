        ...
        <div class='col-xs-12'>
            ...
            <div class="nav-tabs-custom">
                ...
                <ul class="nav nav-tabs" id="myTab">
                    <li class="active"><g:link action="index" controller="mikuShareGetpay">未审核提现</g:link></li>
                            ...
                            <div class="box-body table-responsive no-padding">
                                <g:form action="changeSumGetPayStatusMethod" params="${params}"  id="shipBox">
                                                ...
                                                <tr id="${getPay.id}" class="info">
                                                    ...
                                                    <th class="myth">
                                                        <g:if test="${getPay.tradeNum>0 && (getPay.tradeNum==getPay.successNum || getPay.tradeNum==(getPay.successNum+getPay.failNum)) && getPay.getpayType == 2 as byte}">
                                                            <input type="button" class="btn btn-default" value="微信付款" onclick="getPayMoneyByWx('${getPay?.id}')"/>
                                                        </g:if>
                                                    </th>
                                                    ...
                                                    <th>
                                                        <g:if test="${getPay.tradeNum>0 && (getPay.tradeNum==getPay.successNum || getPay.tradeNum==(getPay.successNum+getPay.failNum))}">
                                                            ...
                                                            <button class="btn btn-sm btn-info" onclick="window.location.replace('/mikuShareGetpay/rollBackGetPay?id=${getPay?.id}');return false;">异常处理</button>
                                                        </g:if>
                                                    </th>
                                                </tr>
                                            </g:each>
                                    ...
                                    <div class="pull-right order-handle">
                                        ...
                                        %{--class="btn btn-sm btn-primary pull-left"><i--}%
                                        %{--class="fa fa-print">全部导出</i></button>&nbsp;&nbsp;&nbsp;&nbsp;--}%
                                        %{--<button type="submit" id="subBtn" class="hide">提交</button>--}%
                                        %{--<button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-truck">&nbsp;审核</i></button>--}%

                                        %{--<button type="button" onclick="sendCheckIds()"--}%
                                        %{--class="btn btn-sm btn-primary"><i--}%
                                        %{--class="fa fa-truck">&nbsp;审核</i></button>--}%

                                        %{--<button type="button" onclick="doInError()"--}%
                                                %{--class="btn btn-sm btn-primary"><i--}%
                                                %{--class="fa fa-truck">&nbsp;作为异常处理</i></button>--}%
                                    </div>
                                </g:form>
                            %{--测试打印--}%
...
//            $('#subBtn').click();
    ...
    $("#max").on("change",function(){
        ...
        var order=$("#order").val();
        var str='/mikuShareGetpay/index?max='+size+"&payType="+payType+"&startTime="+startTime+"&endTime="+endTime+"&agencyMobile="+agencyMobile+"&totalFeeOp="+totalFeeOp+"&targetTotalFee="+targetTotalFee+"&order="+order;
    ...
    function getPayMoneyByWx(id){
        $.ajax({
            ...
            success: function (data) {
                ...
                window.location.replace('/mikuShareGetpay/index');
            },
            error: function (data) {
