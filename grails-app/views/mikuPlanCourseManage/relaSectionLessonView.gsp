...
<script>
    ...
    $(document).on('click','#add_step',function(){
        ...
        if(sectionId){
            $('.loading').show(); //等待进度
            $.ajax({
                url: '/mikuPlanCourseManage/getlessonListAjax',
                data: '',
                type: "POST",
                dataType: "json",
        ...
        ajax_call(getClassSuccess,'/mikuPlanCourseManage/relaSectionLesson',obj,nf);
    ...
    function lessonRelation(info,lessonList,key){
        ...
        for(var i=0;i<lessonList.length;i++){
            console.log('阶段课时关联','i:'+i,lessonList[i].id,sectionLessonInfo.lessonId);
