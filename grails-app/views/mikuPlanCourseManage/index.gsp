            ...
            <table class="table table-hover table-bordered text-center">
                ...
                <g:each status="i" in="${list}" var="li">
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-primary" onclick="window.location.replace('/mikuPlanCourseManage/relaSectionLessonView?id=${li.id}')">关联课时</button>
                            <button class="btn btn-primary" onclick="window.location.replace('/mikuPlanCourseManage/editCourseView?id=${li.id}')">编辑</button>
...
<script>
    ...
    $(document).on('click','.d_btn',function(){
        ...
        if(confirm('确定删除此条记录?')){
            ajax_call(d_success,'/mikuPlanCourseManage/delCourse',data,nf);
        }
