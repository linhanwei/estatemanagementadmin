        ...
        <div class="col-xs-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs" id="myTab">
                    <li class="active"><g:link action="index">经验库列表</g:link></li>
                    <li><g:link action="addView">添加经验库</g:link></li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <div class="header-box form-inline" style="overflow: hidden;">
            <form action="index" class="form-inline content-header">
                <div class="col-lg-4 form-group">
                    ...
                    <input name="stepShortName" class="form-control" value="${params.stepShortName}" type="text">
                </div>

                <div class="col-lg-4 form-group">
                    <label class="control-label">使用规范：</label>
                    <g:select optionKey="key" optionValue="value" name="stepUseStandardFrequency"
                              class="form-control stepUseStandardFrequency" from="${stepUseStandardFrequencyMap}" value="${params.stepUseStandardFrequency}"
                              noSelection="['': '请选择']"/>
        ...
        <div class="table-responsive box-body">
            <table class="table table-hover table-bordered text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>${i+1}</td>
                        <td>${scProblemItemList.get(li?.scProblemId)?.scProblemName}</td>
                        <td>${scThinkingItemList.get(li?.scThinkingId)?.scThinkingName}</td>
                        <td>${li.stepShortName}</td>
                        <td>
                            ${stepUseStandardFrequencyMap.get(li?.stepUseStandardFrequency)}
                        </td>
                        <td>${mikuMineScProductList.get(li?.prodId)?.prodName}</td>
                        <td>
                            <button class="btn btn-primary" onclick="window.location.replace('/mikuCustomizedExperienceDatabase/editView?id=${li.id}')">编辑</button>
...
<script>
    ...
    $(document).on('click','#d_btn',function(){
        ...
        if(confirm('确定删除此条记录?')){
            ajax_call(d_success,'/mikuCustomizedExperienceDatabase/del',data,nf);
        }
