...
import welink.common.MessageSendService
import welink.common.MikuCrowdfund
import welink.common.Order
import welink.common.Trade
import welink.common.TradeDispatchedTrackService
...
class MikuCrowdfundTradeController {
...
//            'in'('status', [2, 4, 6, 5, 7, 9,20] as byte[])
    ...
    def zcFail(){
        ...
        def tradeCode=params.long('tradeCode')
        //找出不成功的众筹订单[2是不成功]
        List<MikuCrowdfund> mikuCrowdfundList=MikuCrowdfund.findAllByStatus(2 as byte)
        //查询符合Trade表操作的数据
        def MyTrade = Trade.createCriteria()
        PagedResultList pagedResultList
        pagedResultList = MyTrade.list(max: params.max ?: 10, offset: params.offset ?: 0) {
...
//            order('id', 'desc')
    ...
    def cancelOneTrade(){
        ...
        redirect(action: 'nopay', controller: 'mikuCrowdfundTrade')
    }
    ...
    def getTradeDetail(Trade trade){
        ...
        //相应的众筹的项目
        MikuCrowdfund mikuCrowdfund=MikuCrowdfund.findById(trade.crowdfundId)
        ...
        tradeDetail.with {
            ...
            it.mcstatus=mikuCrowdfund.status
            it.totalFee=trade.totalFee
            //物流信息
            it.memo=logic.memo
