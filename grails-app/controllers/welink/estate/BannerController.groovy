...
import welink.warehouse.MikuActiveTopic
import welink.warehouse.MikuTopicItem
...
class BannerController {
    ...
    def edit()
    {
        ...
        List<Category> categoryList = Category.findAllByParentIdAndIdGreaterThanAndStatus(null, 20000000L, 1 as byte)
        Long parentId=-1
        MikuActiveTopic topicItem=MikuActiveTopic.findById(banner.topicId)
        if (banner.categoryId)
        {
            if (banner.categoryId>-1)
            {
                //1级类目
                Category cc=Category.findByParentIdAndStatusAndId(null,1 as byte,banner.categoryId)
                if (cc)
                {
                    parentId=banner.categoryId
                }
                else
                {
                    parentId= Category.findById(banner?.categoryId)?.parentId
                }
            }
